...
export const questionApi = {
  // Get question list
  async getQuestions(page: number = 1, pageSize: number = 10): Promise<ApiResponse<Question[]>> {
    try {
      const skip = (page - 1) * pageSize;
      const response = await fetch(`${API_BASE_URL}/question?skip=${skip}&limit=${pageSize}`);
